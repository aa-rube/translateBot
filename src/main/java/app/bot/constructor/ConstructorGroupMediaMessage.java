...
public class ConstructorGroupMediaMessage {
    ...
    public SendMediaGroup getMediaGroupMessage(MediaGroupData data) {
        ...
        if (data.getCaption() != null) {
            try {
                String text = data.getHead() + Formatter.getTextAndEntities(data.getDeeplKey(), data.getCaption(), data.getEntities(), data.getDirection());
                ...
                e.printStackTrace();
                String text = data.getHead() + Formatter.plainText(data.getDeeplKey(), data.getCaption(), data.getDirection());
                media.get(0).setCaption(text);
                return getSendMediaGroupMsg(data.getChatIdToSendMsg(), media);
            }
        }
    ...
    public SendPhoto getMsgWithOnePhotoAndCaption(String authKey, Long chatIdToSendMsg, Update update, String direction, String head) {
        ...
        try {
            ...
            for(var s : markup.getKeyboard()) {
                for (var ss : s) {
                    String keyText = Formatter.plainText(authKey, ss.getText().trim(), direction);
                    ss.setText(keyText);
        ...
        if (update.getMessage().getCaption() != null && update.getMessage().getCaptionEntities() != null) {
            String text = head +  Formatter.getTextAndEntities(authKey, update.getMessage().getCaption(),
                    update.getMessage().getCaptionEntities(), direction);
            return getSendPhoto(chatIdToSendMsg, fileId, text, markup);
        }

        if (update.getMessage().getCaption() != null && update.getMessage().getCaptionEntities() == null) {
            String text = head + Formatter.plainText(authKey, update.getMessage().getCaption(), direction);
            return getSendPhoto(chatIdToSendMsg, fileId, text, markup);
        }
    ...
    public SendDocument getMsgWithOneDocumentAndCaption(String authKey, Long chatIdToSendMsg, Update update, String direction, String head) {
    ...
    public SendAudio getMsgWithOneAudioAndCaption(String authKey, Long chatIdToSendMsg, Update update, String direction, String head) {
        String fileId = update.getMessage().getDocument().getFileId();
